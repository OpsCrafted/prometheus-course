.PHONY: setup verify down clean reset logs-prometheus logs-grafana logs-app verify-rules verify-day-5 verify-day-6 verify-day-7 verify-day-8 verify-day-9 verify-day-10 verify-day-11 verify-day-12 verify-day-13 verify-day-14 verify-day-15 verify-day-16 verify-day-17 verify-day-18 verify-day-19 help
...
help:
	...
	@echo "  make verify-day-19      — Verify Day 19 (Alertmanager routing)"
...
verify-day-19:
	@echo "Verifying Day 19 (Alertmanager routing)..."
	cd labs && docker compose exec prometheus wget -q -O - 'http://alertmanager:9093/api/v1/status' | jq -e '.data.configYAML | test("routes")' > /dev/null && echo "✓ Alertmanager routing config has routes" || (echo "✗ No routes found — update labs/alertmanager.yml and reload"; exit 1)
	cd labs && docker compose exec prometheus wget -q -O - 'http://alertmanager:9093/api/v1/status' | jq -e '.data.configYAML | test("inhibit_rules")' > /dev/null && echo "✓ Alertmanager has inhibition rules" || (echo "✗ No inhibit_rules found — add inhibition to labs/alertmanager.yml"; exit 1)
	cd labs && docker compose exec prometheus wget -q -O - 'http://alertmanager:9093/api/v1/status' | jq -e '.data.configYAML | test("webhook_configs")' > /dev/null && echo "✓ Alertmanager has webhook receiver" || (echo "✗ No webhook_configs found — add a receiver to labs/alertmanager.yml"; exit 1)
